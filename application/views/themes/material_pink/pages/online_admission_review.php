...
// Categorize fields
foreach ($cutom_fields_data as $field) {
    $field_name = strtolower($field->name); // Normalize case

    // CHILD fields: e.g. "Child 1"
    if (preg_match('/^child (\d+)$/i', $field->name, $matches)) {
        $children[(int)$matches[1]] = $field;

    // GUARDIAN fields: e.g. "1st Guardian name", "2nd Guardian relationship"
    } elseif (preg_match('/^(\d+)(st|nd|rd|th) guardian (name|relationship)$/i', $field->name, $matches)) {
        $index = (int)$matches[1];
        $type = strtolower($matches[3]); // 'name' or 'relationship'
        $guardians_grouped[$index][$type] = $field;

    // NEXT OF KIN: e.g. "Next of kin name", "Next of kin contact"
    } elseif (stripos($field->name, 'next of kin') !== false) {
        $next_of_kin[] = $field;
...
// Sort children and guardians
ksort($children);
ksort($guardians_grouped);

// Helper function to display a field
function display_field($field, $customlib) {
    if (!isset($field) || !$customlib->getfieldstatus($field->name)) return;
    ?>
    <div class="col-md-3 col-lg-3 col-sm-3">
        <div class="form-group">
            <label><b><?php echo htmlspecialchars($field->name); ?></b></label>
            <p>
                <?php
                if (is_string($field->field_value) && is_array(json_decode($field->field_value, true)) && (json_last_error() == JSON_ERROR_NONE)) {
                    $field_array = json_decode($field->field_value);
                    foreach ($field_array as $each_value) {
                        echo htmlspecialchars($each_value);
                    }
                } else {
                    $display_field = $field->field_value;
                    if ($field->type === "link") {
                        $display_field = "<a href='" . htmlspecialchars($field->field_value) . "' target='_blank'>" . htmlspecialchars($field->field_value) . "</a>";
                    }
                    echo htmlspecialchars($display_field);
                }
                ?>
            </p>
        </div>
...
<div class="row">
    ...
    foreach ($children as $field) {
        display_field($field, $this->customlib);
    }

    // Guardians
    foreach ($guardians_grouped as $guardian_data) {
        if (isset($guardian_data['name'])) {
            display_field($guardian_data['name'], $this->customlib);
        }
        if (isset($guardian_data['relationship'])) {
            display_field($guardian_data['relationship'], $this->customlib);
        }
    }

    // Next of Kin
    foreach ($next_of_kin as $field) {
        display_field($field, $this->customlib);
    }
